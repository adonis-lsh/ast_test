...
class Plugin {
    constructor(libraryName, libraryDirectory, style, styleLibraryDirectory, customStyleName, camel2DashComponentName, camel2UnderlineComponentName, fileName, customName, transformToDefaultImport, replaceLibraryName, types, index = 0) {
        ...
        this.replaceLibraryName = replaceLibraryName;
        this.types = types;
        ...
        if (!pluginState.selectedMethods[methodName]) {
            ...
            const transformedMethodName = this.camel2UnderlineComponentName // eslint-disable-line
                ...
                : this.camel2DashComponentName
                    ...
                    : methodName;
            const path = winPath(this.customName
                ? this.customName(transformedMethodName, file)
                : (0, path_1.join)(this.libraryName, libraryDirectory, transformedMethodName, this.fileName) // eslint-disable-line
            );
            pluginState.selectedMethods[methodName] = this.transformToDefaultImport // eslint-disable-line
                ? (0, helper_module_imports_1.addDefault)(file.path, path, { nameHint: methodName })
                : (0, helper_module_imports_1.addNamed)(file.path, methodName, path);
            console.log("file.path", file.path);
            // 下面就是导入样式
            if (this.customStyleName) {
                //有值再导入
                if (this.customStyleName(transformedMethodName, file)) {
                    const stylePath = winPath(this.customStyleName(transformedMethodName, file));
                    (0, helper_module_imports_1.addSideEffect)(file.path, `${stylePath}`);
                }
            }
            else if (this.styleLibraryDirectory) {
                const stylePath = winPath((0, path_1.join)(this.libraryName, this.styleLibraryDirectory, transformedMethodName, this.fileName));
                (0, helper_module_imports_1.addSideEffect)(file.path, `${stylePath}`);
            }
            else if (style === true) {
                (0, helper_module_imports_1.addSideEffect)(file.path, `${path}/style`);
            }
            else if (style === "css") {
                (0, helper_module_imports_1.addSideEffect)(file.path, `${path}/style/css`);
            }
    ...
    buildDeclaratorHandler(node, prop, path, state) {
        ...
        const checkScope = (targetNode) => pluginState.specified[targetNode.name] && // eslint-disable-line
            ...
            (path.scope.getBinding(targetNode.name).path.type === "ImportSpecifier" ||
                path.scope.getBinding(targetNode.name).path.type ===
                    "ImportDefaultSpecifier"); // eslint-disable-line
    ...
    ImportDeclaration(path, state) {
        ...
        if (value === libraryName ||
            (this.replaceLibraryName && this.replaceLibraryName(path, pluginState))) {
            node.specifiers.forEach((spec) => {
                var _a;
                if (types.isImportSpecifier(spec)) {
                    pluginState.specified[spec.local.name] = (_a = spec === null || spec === void 0 ? void 0 : spec.imported) === null || _a === void 0 ? void 0 : _a.name;
                }
    ...
    ExportNamedDeclaration(path, state) {
        ...
        node.specifiers.forEach((spec) => {
            if (checkScope(spec.local) &&
                (types.isExportSpecifier(spec) || types.isExportDefaultSpecifier(spec))) {
                const methodName = pluginState.specified[spec.local.name];
                this.importMethod(methodName, file, pluginState);
                const importNode = pluginState.selectedMethods[methodName];
                spec.local.name = importNode.name;
            }
        });
    }
